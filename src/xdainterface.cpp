...
void XdaInterface::rtcmCallback(const rtcm_msgs::Message::ConstPtr &msg)
{
	
	XsMessage rtcm(XMID_ForwardGnssData);
	uint16_t rtcmMessageLength = (const uint16_t)msg->message.size();
	rtcm.setDataBuffer((const uint8_t*)&msg->message[0], rtcmMessageLength, 0);
